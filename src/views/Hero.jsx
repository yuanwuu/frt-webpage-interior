...
import { heroTitle, heroSubtitle, careList } from "../data";
...
const Hero = () => {
  ...
  style={{
    ...
    <div className="relative w-full maxw-[1490px] flex items-center justify-between pt-10 mx-auto px-10 ">
      <img src={logo} alt="" />
      ...
      <img src={cartIcon} className="hidden md:block cursor_pointer text-white " alt="" />
      ...
      `}>
        ...
        <ul className="pt-[60px] items-center flex flex-col gap-8">
          ...
          <NavLink to="products" mobileMenu>Products</NavLink>
          <NavLink to="reference" mobileMenu>Reference</NavLink>
          <NavLink to="care" mobileMenu>Care</NavLink>
        </ul>

        <img src={cartIcon} className="mt8 mx-auto cursor-pointer" alt="" />
      </div>
        ...
        <FadeIn delay={0.4} direction='down' padding fullwidth>
          <h5 className="mt-6 text-clip text-lg xs:text-xl text-white max-w-[500px]">
            {heroSubtitle}
          </h5>
        </FadeIn>
        <FadeIn delay={0.2} direction='up' padding fullwidth>
          <div className="relative w-full xs:w-[460px] mt-11">
            <input type="text" placeholder="Search" className="rounded-full w-full pl-6 pr-[68px] py-4 bg-primary ourline-none text-white text-base xs:text-lg placeholder-white" />
            <img src={searchIcon} className="absolute top-2/4 -translate-y-2/4 right-3 h-11 w-11 cursor-pointer" alt="" />
          </div>
        </FadeIn>
        <div className="absolute h-[50px] xs:-[150px] bottom-0 w-full bg-[linear-gradient(180deg,_#ffffff00_0%,_#FF_100%)] "></div>
